...
void DutVcsBase::init(int argc, char **argv)
{
    ...
    // set VCS cycle to 0
    VcsInit();
    svSetScope(svGetScopeFromName("Cache_top"));
...
void DutVerilatorBase::init(int argc, char **argv)
{
    ...
    this->top = new VCache {contextp};

#if defined(VL_TRACE)
    contextp->traceEverOn(true);
#endif

    svSetScope(svGetScopeFromName("TOP.Cache_top"));
...
int DutVerilatorBase::step(uint64_t ncycle, bool dump)
{
    ...
    if (dump) {
        for (int i = 0; i < ncycle; i++) {
            ((VCache *)(this->top))->eval();
            ((VCache *)(this->top))->contextp()->timeInc(1);
        }
    } else {
        assert(ncycle == 1);
        ((VCache *)(this->top))->eval_step();
    }
...
int DutVerilatorBase::finalize()
{
    ...
    if (this->top != nullptr) {
        ...
        ((VCache *)(this->top))->contextp()->coveragep()->write("VCache_coverage.dat");
#endif
        ((VCache *)(this->top))->final();
        delete (VCache *)(this->top);
        this->top = nullptr;
    }
    return 0;
};
